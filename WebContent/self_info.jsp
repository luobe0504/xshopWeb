<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title></title>
<link type="text/css" rel="stylesheet" href="static/css/mode.css" />
<link type="text/css" rel="stylesheet" href="static/css/self_info.css" />
</head>
<body>

	<div id="outer">

		<div id="lay1">
			<div class="a">
				<a>小米商城│</a>
			</div>
			<div class="a">
				<a>小米商城移动版│</a>
			</div>
			<div class="a">
				<a>问题反馈</a>
			</div>
			<div class="c">
				<a>购物车</a>
			</div>
			<div class="b">
				<a>消息通知</a>
			</div>
			<div class="b">
				<a>注册│</a>
			</div>
			<div class="b">
				<a>登录 |</a>
			</div>
		</div>

		<div id="lay2">
			<div class="x">
				<div class="a">
					<img src="static/img/login.png" />
				</div>
				<div class="b">
					<img src="static/img/search.png" placeholder="搜索内容" />
				</div>
				<div class="b">
					<input />
				</div>
			</div>
			<div class="c">
				<div class="d">
					<div class="e">订单中心</div>
					<div class="f">
						<a href="#">我的订单</a>
					</div>
					<div class="f">
						<a href="#">意外保</a>
					</div>
					<div class="f">
						<a href="#">团购订单</a>
					</div>
					<div class="f">
						<a href="#">评价晒单</a>
					</div>
					<div class="g">
						个人中心
					</div>
					<div class="f">
						<a href="#">我的个人中心</a>
					</div>
					<div class="f">
						<a href="#">消息通知</a>
					</div>
					<div class="f">
						<a href="#">优惠券</a>
					</div>
					<div class="f">
						<a href="#">收货地址</a>
					</div>
				</div>
				<div class="j">
					<div class="h">收货地址</div>
					<form action="#" method="post">
						<div class="z">
							<div class="i">
								<strong>序号</strong>
							</div>
							<div class="i">
								<strong>收件人</strong>
							</div>
							<div class="i">
								<strong>手机号</strong>
							</div>
							<div class="i">
								<strong>地址</strong>
							</div>
							<div class="i">
								<strong>操作</strong>
							</div>
						</div>
						<div class="k">
							收件人<input type="text">
						</div>
						<div class="k">
							手机号<input type="text">
						</div>
						<div class="k">
							详细地址
							<textarea cols="10" rows="100px"></textarea>
						</div>
						<div class="l">
							<input type="submit" value="添加地址" />
						</div>
					</form>
				</div>
			</div>
		</div>

		<div id="lay3">
			<div class="a">小米商城[MIUI米聊)多看书城小米路由器j视频电话小米天猫店小米淘宝直营店|小米网盟|小米移动|[隐私政策[Select
				Region</div>
			<div class="a">@mi.com京ICP证110507号京ICP备10046444号京公网安备11010802020134号京网文[2014]0059-0009号</div>
			<div class="a">违法和不良信息举报电话:185-0130-1238，本网站所列数据，除特殊说明，所有数据均出自我司实验室测试</div>
		</div>
	</div>
</body>
</html>
